...
## Intro
I analyzed KIC-6922244, a hot Jupiter exoplanet of the star Kepler-8b. It is roughly 3,434 years away from the constellation Lyra, and I used this object to detect planetary transits.

## Initial Light Curve
To begin looking at whether or not this star had a transiting exoplanet, I first searched for the lightcurve, and I downloaded it, as well as removing NaNs (not a number values) and normalizing it. Normalizing the light curve is important, as it divides the values of a light curve by its median value. This scales the relative brightness data so that the median flux equals 1.0 (or percentage-based relative units), making it easier to analyze transit or variability signals, rather than having to look at large numbers. I held off on removing outliers for now since I want to get to that later and how it can significantly impact how a light curve can look if you aren't careful. 

I am also using the remove_outliers() function on the lightcurve in order to clean my data by removing things like telemetry glitches or harsh cosmic rays. "Sigma" defines the number of standard deviations a data point's absolute value of flux can be from the median before it is classified as an outlier and is deleted from the graph. Before I chose what value sigma could be, I used the initial light curve, and when I measured the noise, the main band of data (the stellar noise) had a total thickness from about 0.998 to 1.002. This means the standard deviation (σ) is roughly 0.001. As I measured the spikes, I noticed there were positive flare-like spikes shooting up to 1.010 and 1.012. I calculated the distance: A spike at 1.010 is 0.010 away from the center line (1.000). Divide that by the noise (0.010 / 0.001), and I got 10 sigma. Goal: I set sigma=8 or 9 since it is the sweet spot because it is just below the height of those huge cosmic ray or flare spikes (deleting them), but safely above the normal stellar noise band. Going below the values 8 or 9 makes it so that it slowly deletes points where the planet transits, as those points come to be seen as "outliers". Using a low sigma (e.g., sigma=3) will cause Lightkurve to treat the transit dip as "bad" data, deleting those points and completely erasing the planet's signal. In "data," you can see how a light curve will be impossible to read and try to find transits because it skews the data and prevents any viewings of a possibly independent threshold of a transit. It is important to choose the best sigma value to keep the integrity of a straight line.

Alternatively, one can also use sigma_upper and sigma_lower, which split the sigma control into two independent thresholds. Sigma_upper will set the threshold for positive spikes pointing upwards (e.g., cosmic rays, stellar flares, etc.). Sigma_lower will set the threshold for negative spikes pointing downward (e.g., independent light curve eclipses). Ideally, it would be best practice to clean the top spikes aggressively, but keep the bottom transits. For now, I am going to use sigma=9, as it is giving me the cleanest results. 

Looking at the initial light curve, we can tell there is a pattern with the dips in the flux. However, right now it is difficult to determine the orbital period for the planet transiting Kepler-8b. There is still some digging that needs to be done for the light curves. 

## Creating the periodograms

A periodogram (for a light curve) can calculate the power of different frequencies or periods in a stellar light curve, essentially allowing us to find repeating patterns. Lomb-Scargle is useful when trying to find variable stars, exoplanets, etc.

However, it is important to note that there are two methods astronomers can use for a periodogram: the Box Least Squares Method (BLS) or Lomb-Scargle. Before I dive into the definition, let's see what happens when I use each method! When I used the Lomb-Scargle method, I noticed that it peaks at approximately 1.7600 days, while the BLS peaks at around 3.5200, which I also printed in the terminal. I can see that the Lomb-Scargle periodogram also has a lot of strange peaks, not showing a consistent "high peak" like the BLS.

The Lomb–Scargle method asks, “At what frequency does a sinusoidal model best fit the data?” It tests different frequencies and measures how well a sinusoidal model explains the observations at each one. A higher peak indicates stronger evidence for a periodic signal at that frequency. Because Lomb–Scargle searches for sinusoidal variations rather than transit-shaped dips, it can produce multiple prominent peaks from harmonics of the underlying signal, causing it hard to figure out when the planet transits. 

However, the box least squares method is also an algorithm that finds box-shaped features in a star's brightness over time. It tests multiple trial orbital periods, phase-folding the light curve over that period, and fitting a square-down "box" model to find periodic planetary transits. If a "trial" period (a period that is being tested by the BLS) can be matched with the true orbital period, the transit will produce a strong spike on the periodogram, which is why there is a huge peak at 3.5200 days (the period).
...
Understanding why transiting exoplanets rely on BLS rather than the Lomb-Scargle method, I began to test out "folding" the light curve. Folding the light curve stacks the light-curve cycles on top of one another using a specified orbital period. Here, I use the period with the highest power in the periodogram, approximately 3.520 days, because it is the period that best explains the repeating pattern in the data. Folding the light curve is useful because it converts the observation times into orbital phase using the approximate period found by BLS; measurements from different transits can be aligned. The individual transit signals in the light curve, as well as the overall shape and depth easier to measure. I also use the transit time corresponding to the maximum BLS and power, t0_bls, as the reference time when folding the light curve. This places the center of the detected transit at approximately phase 0. Folding converts the observation times into orbital phase, allowing measurements from different transits to be aligned. The individual transit signals then overlap, making the overall transit shape and depth easier to see and measure.
...
Using this relationship, I found a radius ratio (Rp/R*) of 0.090, meaning KIC 6922244's radius is about 9.0% of its host star's radius. This value connects directly back to the transit depth: since Rp/R* = √δ, squaring the radius ratio gives back the depth, 0.090² = 0.0081, or 0.81%. In other words, a radius ratio of 0.090 corresponds to a transit depth of about 0.81%, meaning KIC 6922244 blocks roughly 0.8% of its host star's light during each transit.

# Calculating Planet Radius

The radius ratio gives the size of KIC 6922244 relative to its host star, but it does not give the planet's physical radius by itself. To estimate the planet's actual radius, I need the published radius of the host star, Kepler-8.
